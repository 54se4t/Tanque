...
public class Ventana extends Canvas implements Runnable, KeyListener {
	...
	private static int id; //cuando envia la mensaje de la acción junta con ID para identificar el jugador
	...
	public void send() { //Enviar la acción
		...
		for (int i = 0; i < leerdatos.tanqueId.size(); i++) {
			...
			int tanqueY = (int) (getHeight()*fondo.TIERRA) - leerdatos.tanqueAlto.get(i);
				if (leerdatos.vida.get(i) <= 0) {
					...
					g.setColor(Color.black);//ca�on de tanque
				...
				/* 
				 * No hace nada cuando sucede este excepción porque la clase LeerDatos esta siempre
